...
function collectLayouts(pathname: string): LayoutComponent[] {
  ...
  for (const segment of rawSegments) {
    // Gruppenordner nicht mehr überspringen, sondern Layouts auch dort laden
    currentLayoutDir =
      currentLayoutDir === "./"
        ? `./${segment}/`
...
export function AppRouter({ url }: { url: URL }) {
  ...
  const transformed: RouteEntry[] = Object.entries(pageModules).map(
    ([pagePath, pageContent]) => {
      const { route, layoutPath } = filePathToRoutePattern(pagePath, "./")
      return {
        pagePath,
        route,
        module: pageContent,
        layouts: collectLayouts(layoutPath),
      }
    },
  )
