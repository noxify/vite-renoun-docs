...
interface PageModule {
  default: unknown
  generateStaticParams?: StaticPathFn
}
...
export async function getStaticRoutes() {
  ...
  const transformed = await Promise.all(
    Object.entries(staticPathModules).map(async ([pagePath, pageModule]) => {
      ...
      if (typeof module.generateStaticParams === "function") {
        staticPathsFn = module.generateStaticParams
      }

      const paths: (StaticPathObject | string)[] = staticPathsFn
        ? await staticPathsFn()
        : ["/"] // Fallback for static pages without parameters
      return {
        pagePath,
        route,
        staticPaths: paths,
  ...
  for (const entry of transformed) {
    ...
    for (const p of staticPaths) {
      ...
      if (typeof p === "object" && p !== null) {
        finalPath = fillDynamicRoute(route.route, p)
      } else if (p === "/" || p === "") {
        finalPath = route.route === "/" ? "/" : route.route.replace(/\/$/, "")
      } else {
        // fallback for legacy string param
        finalPath = fillDynamicRoute(route.route, { slug: p })
      }
    ...
    treeView[route.route] = route.route.includes("[") ? generated : []
  }
